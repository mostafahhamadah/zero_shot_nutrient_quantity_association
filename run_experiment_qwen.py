"""
run_experiment_qwen.py
======================
Experiment 4 — Qwen 2.5:7b Semantic Classifier
...
SCIENTIFIC QUESTION
-------------------
Can a general-purpose multilingual LLM outperform a specialised biomedical
span-extraction model (GLiNER) and the rule-based classifier for zero-shot
semantic classification of supplement label tokens?

Controlled variable: Stage 3 (classifier) only.
Stages 1, 2, 4, 5 are byte-for-byte identical to Experiment 2v1.

CHANGES FROM run_experiment_gliner.py (Experiment 3)
-----------------------------------------------------
  Stage 3 : src/classification/qwen_classifier.py    (was gliner_classifier)
            ...
            Serving sz: labelled CONTEXT directly by Qwen
            Fallback  : UNKNOWN → rule-based fallback chain
  Stages 1, 2, 4, 5 : UNCHANGED from Experiment 2v1

USAGE
-----
  python run_experiment_qwen.py
  python run_experiment_qwen.py --no-llm
  python run_experiment_qwen.py --baseline outputs/experiment_paddle_13_llm/evaluation_results.json
  python run_experiment_qwen.py --qwen-model qwen2.5:14b
"""
...
EXPERIMENT_NAME  = "experiment_04_qwen_classifier"
ANNOTATIONS_DIR  = Path("data/annotations")          # ← JSON source of truth
IMAGE_DIR        = Path("data/raw")
BASELINE_JSON    = Path("outputs/experiment_paddle_13_llm/evaluation_results.json")
QWEN_MODEL       = "qwen3.5:4b"   # Stage 3 classification — lighter model
EVAL_MODEL       = "qwen2.5:7b"   # Stage 6 evaluation — stronger model for judgment
IMAGE_EXTENSIONS = {".jpg", ".jpeg", ".png"}

logging.basicConfig(level=logging.INFO, format="%(levelname)s | %(message)s")
logger = logging.getLogger("run_experiment_qwen")
...
def _run_stage3_qwen(tokens: list, classifier: QwenClassifier) -> tuple[list, dict]:
    """Stage 3: Qwen 2.5:7b semantic classifier."""
    ...
    matched_scores   = [t["qwen_score"] for t in labelled if t.get("qwen_score") is not None]
    matched_entities = {t.get("qwen_entity") for t in labelled if t.get("qwen_entity") is not None}
    ...
    diag = {
        ...
        "qwen_matched":          len(matched_entities),
        "qwen_avg_score":        (round(sum(matched_scores) / len(matched_scores), 4)
...
def _accumulate_diag(agg: dict, img_id: str,
    ...
    agg["qwen_calls"]            += 1
    agg["qwen_matched"]          += d3["qwen_matched"]
    if d3["qwen_avg_score"] > 0:
        agg["qwen_score_sum"] += d3["qwen_avg_score"] * d3["qwen_matched"]
        agg["qwen_score_n"]   += d3["qwen_matched"]
    ...
    if not qwen_ok:
        agg["qwen_failed_images"].append(img_id)
...
def _print_diagnostics(agg: dict, n_images: int) -> None:
    ...
    q_avg = (round(agg["qwen_score_sum"] / agg["qwen_score_n"], 4)
             if agg["qwen_score_n"] > 0 else 0.0)
    ctx_t = agg["clf_CONTEXT"]

    print(f"\n{'='*W}")
    print("  PIPELINE DIAGNOSTICS — Experiment 4 (Qwen 2.5:7b)")
    print(f"{'='*W}")
    ...
    print()
    print(f"  ── Stage 3 (Qwen) ───────────────────────────────────")
    print(f"  Total tokens            : {clf_t}")
    print(f"  Ollama calls made       : {agg['qwen_calls']}")
    print(f"  Entities matched        : {agg['qwen_matched']}")
    print(f"  Mean match score        : {q_avg:.4f}")
    if agg["qwen_failed_images"]:
        print(f"  Failed Qwen calls       : {agg['qwen_failed_images']}")
    print()
...
def _save_diagnostics_csv(agg: dict, out_dir: Path) -> None:
    ...
    row = {
        ...
        "qwen_calls":            agg["qwen_calls"],
        "qwen_matched":          agg["qwen_matched"],
        "qwen_avg_score":        q_avg,
        "qwen_failed_images":    len(agg["qwen_failed_images"]),
...
def _print_summary(current: dict, baseline_path: Path) -> None:
    ...
    if baseline:
        ...
        title  = "EXPERIMENT 4 SUMMARY (Qwen)"
...
def run_experiment(
    ...
    logger.info("Initialising Qwen classifier: model=%s", qwen_model)
    classifier    = QwenClassifier(model_id=qwen_model)
    graph_builder = GraphConstructor()
    associator    = TupleAssociator()

    logger.info("Warming up Qwen (loading model into memory)...")
    import requests as _req
    try:
        _req.post("http://localhost:11434/api/generate",
                  json={"model": qwen_model, "prompt": "say ready", "stream": False},
                  timeout=300)
        logger.info("Qwen warmup complete.")
    except Exception:
        logger.warning("Qwen warmup failed — model may be slow on first image.")
    ...
    for img_path in images:
        ...
        try:
            ...
            labeled_tokens, d3    = _run_stage3_qwen(corrected_tokens, classifier)
            if d3["NUTRIENT"] == 0 and d3["QUANTITY"] == 0 and d3["UNIT"] == 0:
                qwen_ok = False
                logger.warning("Qwen returned no entities for %s — all UNKNOWN.", image_id)
            ...
            d3 = {"total_tokens": 0, "NUTRIENT": 0, "QUANTITY": 0, "UNIT": 0,
                  ...
                  "context_norm_missing": 0, "qwen_matched": 0, "qwen_avg_score": 0.0}
        ...
        _accumulate_diag(agg_diag, image_id, d1, d2, d3, d4, d5, qwen_ok)
    ...
    metrics   = evaluator.run(
        ...
        out_dir     = out_dir,
        notes       = (
            f"Qwen {qwen_model} Stage 3 classifier. "
            ...
            f"Fallback: UNKNOWN -> rule-based chain. "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Qwen 2.5:7b classifier experiment — Experiment 4")
