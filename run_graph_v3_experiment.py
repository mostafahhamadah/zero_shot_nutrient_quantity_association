...
================================
Runs the V3 pipeline end-to-end using the embedding classifier in
...
CHANGES (exp39 edge modes)
--------------------------
  - --row-edge-mode {cy|rank|role_rank}  and  --col-edge-mode {cx|column_id}
    are forwarded into GraphConstructorV2. Defaults reproduce the
    current geometric behaviour (cy/cx).

CHANGES (late-fusion merge)
---------------------------
  - --merge-modes runs DEFAULT (cy) first, then STRUCTURAL (role_rank),
    and adds ONLY role_rank's UNIQUE tuples to the default set. The default
    (cy) result is never altered; role_rank can only ADD tuples the default
    run missed. Overrides --row-edge-mode. Implemented via
    src/matching/merged_graph_associator.py (MergedModeAssociator).

CHANGES (stage audit)
---------------------
  - --audit writes, per image, the OUTPUT of every stage to its own
    file under outputs/{EXPERIMENT}/audit/{image}/ plus a
    _STAGE_AUDIT.md stating each stage's input and output. Each stage's
...
--------
  ...
  Stage 4   — Graph V2  (row/col edge mode configurable; dual when --merge-modes)
...
-----
    ...
    python run_embedding_only_experiment.py --experiment exp39a_colid \
        --col-edge-mode column_id
    python run_embedding_only_experiment.py --experiment exp39b_rank_colid \
        --row-edge-mode rank --col-edge-mode column_id

    # late-fusion merge (default cy + role_rank unique-add):
    python run_embedding_only_experiment.py --experiment exp39f_merged \
        --merge-modes --no-llm --compare exp39d

    # with per-stage audit dumps (scope to a few images for inspection):
    python run_embedding_only_experiment.py --experiment exp39b_audit \
        --row-edge-mode rank --col-edge-mode column_id --audit --images 1,108,118
"""
...
def write_image_audit(audit_root, image_key, img_path,
    ...
    md.append("- **Schema:** unchanged; `token` text cleaned/normalised, "
              "tokens may be split (e.g. energy `kJ/kcal`)")
    md.append(f"- **Sample correction log:**\n```json\n{_sample(correction_log or [], n=3)}\n```\n")

    md.append("## Stage 3 — Semantic Classification  ·  `embedding_semantic_classifier.py` (embedding_only)")
...
parser.add_argument('--row-edge-mode', default='cy', choices=['cy', 'rank', 'role_rank'],
                    help="ROW_COMPAT build: 'cy' geometric | 'rank' structural | 'role_rank' label-specific rank")
parser.add_argument('--col-edge-mode', default='cx', choices=['cx', 'column_id'],
                    help="COL_COMPAT build: 'cx' geometric (default) | 'column_id' structural")
# ── late-fusion merge ────────────────────────────────────────────────────────
parser.add_argument('--merge-modes', action='store_true',
                    help="Run DEFAULT (cy) then STRUCTURAL (role_rank) and ADD ONLY "
                         ...
                         "--row-edge-mode. col_mode still applies to both passes.")
# ── per-stage audit ─────────────────────────────────────────────────────────
...
print(f"  Pipeline   : V2 (PaddleOCR + Corrector V2 + Embedding Clf + Graph V2)")
...
if MERGE_MODES:
    print(f"  Graph edges: MERGE (default cy + structural role_rank, add unique) "
          f"| col_mode={COL_EDGE_MODE}")
else:
    print(f"  Graph edges: row_mode={ROW_EDGE_MODE} | col_mode={COL_EDGE_MODE}")
...
# internally, both at the chosen col_mode, and keeps default + role_rank uniques.
merger = MergedModeAssociator(col_edge_mode=COL_EDGE_MODE) if MERGE_MODES else None
...
with open(LOG_FILE, 'w', encoding='utf-8') as f:
    ...
    f.write(f"Pipeline     : V2 + Corrector V2 + EmbeddingSemanticClassifier (embedding_only)\n")
