"""
run_embedding_only_rank_first_experiment.py
================================
Runs the Graph V2 + Rank-First Association pipeline end-to-end using the embedding classifier in
...
--------
  ...
  Stage 4   — Graph V2
  Stage 5   — Rank-First Association (+ paragraph fallback)
  Stage 6   — LLM-Assisted Tuple Evaluator

USAGE
-----
    python run_embedding_only_rank_first_experiment.py --experiment exp_rank_first
    python run_embedding_only_rank_first_experiment.py --experiment exp_rank_first_t60 --threshold 0.60
    python run_embedding_only_rank_first_experiment.py --experiment exp_rank_first_quick --no-llm
    python run_embedding_only_rank_first_experiment.py --experiment exp_rank_first --compare exp40_embonly_tuned
"""
...
from src.matching.association_rank_first import TupleAssociatorRankFirst
...
print(f"  Pipeline   : Graph V2 + Rank-First Association (PaddleOCR + Corrector V2 + Embedding Clf)")
...
associator  = TupleAssociatorRankFirst()
...
for img_path in IMAGE_FILES:
    ...
    try:
        ...
        diagnostics[image_key] = {
            ...
            'assoc_mode':          assoc_diag.get('mode', 'rank_first'),
            'assoc_methods':       assoc_methods,
            'assoc_rank_first':    assoc_methods.get('rank_first', 0),
            'assoc_row_fallback':  assoc_methods.get('row_fallback', 0),
            'assoc_col_fallback':  assoc_methods.get('col_fallback', 0),
            'assoc_score_fallback':assoc_methods.get('global_score_fallback', 0),
            'assoc_no_quantity':   assoc_methods.get('no_quantity', 0),
        ...
        print(f"OK    {image_key}  -> {len(tuples):>3} tuples  "
              ...
              f"RANK:{assoc_methods.get('rank_first',0):>3}  "
              f"ROW_FB:{assoc_methods.get('row_fallback',0):>3}  "
              f"COL_FB:{assoc_methods.get('col_fallback',0):>3}")
        ok += 1
...
with open(LOG_FILE, 'w', encoding='utf-8') as f:
    ...
    f.write(f"Pipeline     : Graph V2 + Rank-First Association + Corrector V2 + EmbeddingSemanticClassifier (embedding_only)\n")
    ...
    f.write("Association  : TupleAssociatorRankFirst\n")
    f.write("Matching     : rank-first -> row fallback -> column fallback -> global score fallback\n\n")
